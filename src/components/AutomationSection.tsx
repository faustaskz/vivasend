...
import { fadeUp, staggerContainer, slideInLeft, slideInRight, cardVariant, viewportOptions } from '@/lib/animations'
      ...
      <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOptions}>
          ...
          <motion.p variants={fadeUp} className="text-[16px] text-vs-muted leading-[1.7] mb-6">
            Automatizuota VivaSend platforma dirba už jus 24/7. Nustatykite taisykles vieną kartą – sistema pati siųs reikiamus laiškus reikiamu metu.
          </motion.p>
        ...
        <motion.div variants={slideInRight} initial="hidden" whileInView="visible" viewport={viewportOptions} className="bg-bg-3 border border-vs-border2 rounded-2xl p-7">
          <p className="text-[11px] text-vs-faint uppercase tracking-[0.08em] mb-4">Automatizacijos scenarijus</p>
          <motion.div
          ...
          >
            {FLOW_STEPS.map((s, i) => (
              <motion.div key={s.name} variants={cardVariant} className={`flex gap-4 items-start py-4 ${i > 0 ? 'border-t border-vs-border' : ''}`}>
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-sm flex-shrink-0 ${s.status === 'done' ? 'bg-[rgba(39,201,63,0.15)] text-[#27c93f]' : s.status === 'active' ? 'bg-orange text-white' : 'bg-bg-4 text-vs-muted'}`}>
                  {s.icon}
                </div>
                <div>
