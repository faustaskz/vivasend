...
const PLANS = [
  {
    name: 'Standard',
    desc: 'Esminiai el. pašto įrankiai modernaus verslo plėtrai internetu',
    price: '9,30',
    sub: 'nuo 2 500 prenumeratorių',
    featured: false,
    cta: 'Pradėti',
  ...
  {
    name: 'Pro',
    desc: 'Išplėstinės daugiakanalės funkcijos ir aukščiausios kokybės palaikymas',
    price: '27,23',
    sub: 'nuo 2 500 prenumeratorių',
    featured: true,
    cta: 'Pradėti',
  ...
  {
    name: 'Enterprise',
    desc: 'Didelėms organizacijoms – personalizavimas ir automatiniai sprendimai',
    price: null,
    sub: 'kaina pagal poreikius',
    featured: false,
    cta: 'Gauti pasiūlymą',
    ctaHref: CONTACT_URL,
    features: [
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {PLANS.map((plan) => (
            <div
              key={plan.name}
              className={`flex flex-col bg-bg-3 border rounded-2xl p-8 transition-all duration-300 hover:-translate-y-1 ${
                plan.featured
                  ? 'border-orange bg-gradient-to-b from-orange/7 to-bg-3'
                  : 'border-vs-border hover:border-vs-border2'
              }`}
            >
              {plan.featured && (
                <span className="self-start text-[11px] font-semibold tracking-wider uppercase bg-orange text-white px-3 py-1 rounded-full mb-5">
                  Populiariausias
                </span>
              )}
              ...
              <div className="pb-7 mb-7 border-b border-vs-border">
                {plan.price ? (
                  <>
                    ...
                    <span className="text-[14px] text-vs-muted">/mėn.</span>
                  </>
                ) : (
              ...
              <a
                ...
                className={`block text-center py-3.5 rounded-[10px] text-[15px] font-medium transition-all ${
                  plan.featured
                    ? 'bg-orange text-white hover:bg-orange-dark'
                    : 'border border-vs-border2 text-vs-text hover:bg-bg-4'
                }`}
              >
                {plan.cta}
              </a>
